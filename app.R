...
main_page <- tabPanel(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("num_var_2", "Numerische Variable", choices = c(not_sel)),
      selectInput("fact_var", "Kategorielle Variable", choices = c(not_sel)),
      br(),
      actionButton("run_button", "Starte die Datenanalyse", icon = icon("play"))
    ),
    mainPanel(
      title = "Plot",
      h3("Diagramm"),
